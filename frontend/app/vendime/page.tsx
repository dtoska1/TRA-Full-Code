...
async function getVendimeFeed(options: {
  municipality: string;
  year: string;
  sort: "newest" | "oldest";
}): Promise<{
  ...
  try {
    ...
    return {
      data: null,
      error: error instanceof Error ? error.message : "Failed to load vendime feed",
    };
  }
}
  ...
  return (
    <main className="mx-auto w-full max-w-5xl p-4 pb-10 sm:p-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Vendime</p>
        <h1 className="mt-2 text-2xl font-semibold text-slate-900">Vendime</h1>
        <p className="mt-2 text-sm text-slate-600">
          Vendimet e këshillave bashkiake nga të gjitha bashkitë e Shqipërisë.
        </p>
